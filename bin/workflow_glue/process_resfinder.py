...
def process_resfinder(resfinder_data, amr_type="resfinder"):
    ...
    resfinder_data = resfinder_data.assign(Start=loc_split[0], End=loc_split[1])
...
def read_pointfinder_xml(xml_file):
    ...
    if results_df.loc[0, "query_start"] > results_df.loc[0, "query_end"]:
        results_df["query_start"], results_df["query_end"] = (
            results_df["query_end"],
            results_df["query_start"],
        )
        results_df["subject_start"], results_df["subject_end"] = (
            results_df["subject_end"],
            results_df["subject_start"],
        )
    return results_df
...
def convert_pointfinder_row(database_location, row):
    ...
    pointfinder_blast_data[["Global_loc_bp", "Global_loc_bp_end"]] = (
...
def process_pointfinder(pointfinder_data, database_location):
    ...
    loc_split = pointfinder_data["Mutation"].str.split(" ", n=1, expand=True)
    ...
    for _, row in pointfinder_data.iterrows():
        pointfinder_blast_data = convert_pointfinder_row(database_location, row)
        df_list.append(pointfinder_blast_data)

    concat_df = pd.concat(df_list)
    concat_df_merge = concat_df.merge(pointfinder_data, on="Mutation")
    concat_df_merge_tidy = concat_df_merge.iloc[:, [4, 8, 9, 12, 10, 11, 13, 14]]
...
def main(args):
    ...
    if args.disinf_file != "NOT_RUN":
        ...
        disinf_results = process_resfinder(disinf_data, amr_type="Disinfectant")
    else:
        disinf_results = pd.DataFrame()
    amr_results = pd.concat([resfinder_results, pointfinder_results, disinf_results])
...
def argparser():
    ...
    parser.add_argument(
        "--resfinder_file", help="Resfinder tab delimited results file."
    )
    parser.add_argument(
        "--disinf_file",
        default="NOT_RUN",
    ...
    parser.add_argument(
        "--database_location", help="Location of pointfinder results tmp storage."
    )
    parser.add_argument(
        "--species_id",
        help="Species id for pointfinder database" "(e.g mycobacterium_tuberculosis).",
    )
    parser.add_argument(
        "--output",
        default="Results_tab_processed.txt",
