...
def argparser():
    ...
    parser.add_argument("--metadata", help="Workflow meta map json")
    parser.add_argument("--checkpoint_data", help="Checkpoint data to process")
    parser.add_argument("--output_definitions", help="Workflow output definitions")
...
def make_checkpoints_file(args):
    ...
    checkpoints = dict(files=dict(), checkpoints=dict())
        ...
        for new_checkpoint_data in new_checkpoint_datas:
            if checkpoint == new_checkpoint_data["checkpoint_name"]:
                ...
                if output_schema["checkpoints"][checkpoint]["type"] == "per-sample":
                    details[new_checkpoint_data["sample"]] = new_checkpoint_data[
                        "status"
                    ]
                else:
                    checkpoints["checkpoints"][checkpoint] = new_checkpoint_data[
                        "status"
                    ]
